...
import { getArticles,postArticles,deleteArticles } from "../API/knowledgeBase";
import ClayTable from "@clayui/table";
import ClayButton from "@clayui/button";
import ClayModal, { useModal } from "@clayui/modal";
import ClayForm, { ClayInput } from "@clayui/form";

export default function CRUDaricles() {
  const [aricles, setaricles] = useState([]);
  ...
  const handleDelete = (id) => {
    deleteArticles(id).then(() => {
      setaricles((prevaricles) => prevaricles.filter((aricle) => aricle.id !== id));
    });
  };
  
  const Addaricles = async () => {
    try {
      await postArticles(title, articleBody);
    } catch (error) {
      console.error("Error creating aricle:", error);
    }
  };
    ...
    <>
      {open && (
        <ClayModal
          ...
          status="info"
        >
          <ClayModal.Header>Add Articles</ClayModal.Header>
          <ClayModal.Body>
            <ClayForm.Group>
              <label htmlFor="basicInputText" >Title</label>
              <ClayInput placeholder="Insert your Title here" type="text" value={title} onChange={(e) => settitle(e.target.value)}/>
            </ClayForm.Group>
        ...
        {isLoading ? (
          ...
          <div>
            ...
            <ClayTable>
              ...
              <ClayTable.Body>
                {aricles.map((aricle, index) => (
                  <ClayTable.Row key={index}>
                    <ClayTable.Cell headingTitle>{aricle.id}</ClayTable.Cell>
                    <ClayTable.Cell>{aricle.title}</ClayTable.Cell>
                    <ClayTable.Cell>{aricle.articleBody}</ClayTable.Cell>
                    ...
                    <ClayTable.Cell>
                      {
                        <ClayButton
                          displayType="secondary"
                          onClick={() => handleDelete(aricle.id)}
                        >
                          <svg
                            fill="#000000"
                            height="24px"
